...
class VS_embedding(object):
    ...
    def _image_encoder(self):
        ...
        img_encode = tf.nn.l2_normalize(img_encode, 1)
        return img_encode
    ...
    def _caption_encoder(self):
        ...
        #return cap_encode
        cap_encode = tf.nn.l2_normalize(state, 1)
        return cap_encode

    def _loss(self):
        img_encode = self._image_encoder()
        cap_encode = self._caption_encoder()
        simi = self._similarity(img_encode, cap_encode)
        ...
        for i in range(1, self.batch_size+1):
            trans_img_encode.append(img_encode[-i])

        wrong_smi = self._similarity(trans_img_encode, cap_encode)
        return loss, simi, wrong_smi
